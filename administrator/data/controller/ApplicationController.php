<?php
    ...
    else if ($action == "loadPrintTable")
    {
        ...
        $result = $Sql->getApplicantPrintId($request);
    ...
    else if ($action == "loadApprovedIdTable")
    {
        $applicantType = $_POST['applicantType'];
        $barangay = $_POST['barangay'];
        ...
        $result = $Sql->getApplicantApprovedId($request);
        ...
        foreach($result as $data)
        {
            ...
            $tableRow .= "<td>" . $data['FULL_NAME'] . "</td>";
            $tableRow .= "<td>" . $data['BARANGAY'] . "</td>";
            $tableRow .= '<td><button type="button" id="print" onclick="PrintID.clickView(`'. $data['PERSON_ID'] .'`, `'.$data['APPLICANT_TYPE'].'`)" class="btn btn-success btn-sm"><i class="bi bi-eye"></i> View </button></td>';
            $tableRow .= "</tr>";
            $counter++;
        }
