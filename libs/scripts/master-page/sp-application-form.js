...
const SP = (() => {
    ...
    thisSP.approveApplication = () => {
      ...
      if (
        formData.get("barangayCertificate")["name"] == "" ||
        formData.get("validID")["name"] == "" ||
            ...
            success: function (data) {
                ...
                if (status == "Pending") {
                    if(administratorRole == "Main Administrator") {
                        ...
                        <button class="btn btn-danger w-100" type="button" name="reject" onclick="SP.reject('${data[0]["PERSON_ID"]}', '${applicationType}');");">Reject</button>`;
                    }
